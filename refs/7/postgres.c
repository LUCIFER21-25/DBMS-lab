...
{
	const char* prefix = "insert into r values";
	int flag = 1;
	for(int i = 0; i < 20; i++)
	{
	...
	if(flag)
	{
		...
		char*k = (char*)malloc(strlen(query_string)-19);
		i++;
		int j = 0;
		...
		k[j] = '\0';
		char* z = malloc(2*strlen(query_string)+20);
		sprintf(z, "%s insert into delta_r values ('I',%s", query_string, k);
		query_string = z;
	}
